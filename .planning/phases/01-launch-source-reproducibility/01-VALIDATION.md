...
| 01-03-01 | 03 | 2 | PLAT-02 | T-01-09 | UI and API are served from the same containerized origin | e2e | `cd /home/lnayyar/projects/CourseAIVibeCodeUdmey/finally && python3 scripts/smoke_start_command.py && cd test && docker compose -f docker-compose.test.yml run --rm playwright npx playwright test specs/launch.spec.ts specs/reproducibility.spec.ts` | Planned in 01-03 | ⬜ pending |
| 01-03-02 | 03 | 2 | PLAT-01 | T-01-10 | The documented student start command is verified on the success path and the full Playwright harness remains repeatable against the rebuilt Docker runtime | e2e/full | `cd test && docker compose -f docker-compose.test.yml up --abort-on-container-exit --exit-code-from playwright` | Planned in 01-03 | ⬜ pending |
